...
def part1():
    ...
    for move in input:
        ...
        amount = int(move[1:])
        if op == 'R':
            curr_deg = dir_map[curr_dir]
            curr_dir = deg_map[(curr_deg + amount) % 360]
        elif op == 'L':
            curr_deg = dir_map[curr_dir]
            curr_dir = deg_map[(curr_deg - amount) % 360]
        else:
            move_dir = curr_dir if op == 'F' else op
            if move_dir ==  'N':
                ship = (ship[0],ship[1] + amount)
            if move_dir ==  'E':
               ship = (ship[0] + amount,ship[1])
            if move_dir ==  'S':
                ship = (ship[0],ship[1] - amount)
            if move_dir ==  'W':
                ship = (ship[0] - amount,ship[1])
...
def part2():
    ...
    for move in input:
        ...
        amount = int(move[1:])
        if op == 'F':
            ship = (ship[0] + wp[0] * amount, ship[1] +wp[1]* amount)
        if op ==  'N':
            wp = (wp[0],wp[1] + amount)
        if op ==  'E':
            wp = (wp[0] + amount,wp[1])
        ...
        if op ==  'W':
            wp = (wp[0] - amount,wp[1])
        if op == 'R':
            wp = rotate(wp, math.radians(-amount))
            no_op
        elif op == 'L':
            wp = rotate(wp, math.radians(amount))
            no_op
